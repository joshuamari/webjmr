...
try {
    ...
    if ($entriesStmt->rowCount() > 0) {
        ...
        foreach ($entriesArr as $entries) {
            ...
            $rawIn = $entries["in"];
            $rawOut = $entries["out"];
            $in_timestamp = stripSeconds(strtotime($rawIn));
            $out_timestamp = stripSeconds(strtotime($rawOut));
            $in_time = date("H:i ",  $in_timestamp);
            $out_time = date("H:i ", $out_timestamp);
...
function getReductions($start, $end, $location, $currentday)
{
    ...
    global $wfhID;
    $reductions = 0;
    $start = date("H:i", stripSeconds(strtotime($start)));
    $end = date("H:i", stripSeconds(strtotime($end)));
    $end_stamp = stripSeconds(strtotime($end));
    ...
    if ($redStmt->rowCount() > 0) {
        ...
        foreach ($redArr as $red) {
            $starttime = $red["start"];
            $endtime = $red["end"];
            $start_timestamp = stripSeconds(strtotime($starttime));
            $end_timestamp = stripSeconds(strtotime($endtime));
...
function isWorkDay($selDate, $loc)
{
    ...
    $workDayQ = "SELECT fldHolidayType FROM kdtholiday WHERE fldDate = :selDate AND fldLocID = :loc";
...
function stripSeconds($timestamp) {
