...
$(document).ready(function () {
  ...
  //imagine, eto yung query pag nakaselect na yung group, members at month
...
function pGet(empNum, ifOT) {
  ...
  returnObject = [...new Map(returnObject.map(x => [x["pIndex"], x])).values()];
  return returnObject;
};
...
function addCells() {
  ...
  $('.pRow,.oRow,.tRow,.lRow,.lTot,.tTot,.lTot,.oTot,.gpRow,.goRow,.goTot,.gtTot,.glRow,.glTot').append(addHtml)
};
//#endregion
...
function fillTable(entry) {
  if (!Leaves.includes(entry["pIndex"])) {
    $($(`.${ifOT(entry["OT"])}Row[p-index="${entry["pIndex"]}"][employee-number="${entry["empNum"]}"]`).children()[parseInt(entry["entryDate"]) + 1]).text(entry["hours"]);
  } else {
    if (oLeaves.hasOwnProperty(entry["pIndex"])) {
      $($(`.lRow[p-index="others"][employee-number="${entry["empNum"]}"]`).children()[parseInt(entry["entryDate"]) + 1]).text(entry["hours"]);
    }
    $($(`.lRow[p-index="${entry["pIndex"]}"][employee-number="${entry["empNum"]}"]`).children()[parseInt(entry["entryDate"]) + 1]).text(entry["hours"]);
  }
}
...
function getTotals() {
  ...
  // OT
  $('.oTot').each(function () {
    var getEmpOts = $(`.oRow[employee-number="${$(this).attr('employee-number')}"]`)
    for (let x = 2; x < $(this).children().length - 1; x++) {
      var totalOT = 0;
      $(getEmpOts).each(function () {
        totalOT += parseFloat($($(this).children()[x]).text() == "" ? 0 : $($(this).children()[x]).text());
      })
  ...
  // Reg + OT
  $('.tTot').each(function () {
    var getEmpTots = $(`.pRow[employee-number="${$(this).attr('employee-number')}"]`)
    for (let x = 2; x < $(this).children().length - 1; x++) {
      var totaltime = 0;
      $(getEmpTots).each(function () {
        totaltime += parseFloat($($(this).children()[x]).text() == "" ? 0 : $($(this).children()[x]).text());
      })
      totaltime += parseFloat($($(`.oTot[employee-number="${$(this).attr('employee-number')}"]`).children()[x]).text() == "" ? 0 : $($(`.oTot[employee-number="${$(this).attr('employee-number')}"]`).children()[x]).text());
  ...
  // Leaves
  $('.lTot').each(function () {
    var getEmpLeaves = $(`.lRow[employee-number="${$(this).attr('employee-number')}"]`)
    for (let x = 2; x < $(this).children().length - 1; x++) {
      var totalLeaves = 0;
      $(getEmpLeaves).each(function () {
        totalLeaves += parseFloat($($(this).children()[x]).text() == "" ? 0 : $($(this).children()[x]).text());
      })
  ...
  $(".gpRow").each(function () {
    ...
    for (let x = 2; x < $(this).children().length - 1; x++) {
      ...
      $(getpRows).each(function () {
        gpTotal += parseFloat($($(this).children()[x]).text() == "" ? 0 : $($(this).children()[x]).text())
      })
  ...
  $(".goRow").each(function(){
    ...
    for (let x = 2; x < $(this).children().length - 1; x++) {
      ...
      $(getoRows).each(function () {
        goTotal += parseFloat($($(this).children()[x]).text() == "" ? 0 : $($(this).children()[x]).text())
      })
  ...
  $(".glRow").each(function(){
    ...
    for (let x = 2; x < $(this).children().length - 1; x++) {
      ...
      $(getlRows).each(function () {
        glTotal += parseFloat($($(this).children()[x]).text() == "" ? 0 : $($(this).children()[x]).text())
      })
  ...
  $('.tCell').each(function () {
    ...
    for (let x = 0; x < _maxDays; x++) {
      rightTot += parseFloat($(getLeft[x]).text() == "" ? 0 : $(getLeft[x]).text());
    }
    $(this).text(rightTot);
  })
}
