...
$ogpFilter = ""; // no longer filter OGP in SQL
#endregion
...
foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $entry) {
    ...
    // 🧠 Compute isHiram only if not Leave
    $isHiram = false;
    if (!$isLeave) {
        $isHiram = (
    ...
    $targetKey = $isLeave ? "Leaves" : ($isOT ? "OTEntries" : "RegularHourEntries");

    // Insert into RegularHours regardless of OT/Leave (to match previous behavior)
    if ($targetKey !== "Leaves") {
