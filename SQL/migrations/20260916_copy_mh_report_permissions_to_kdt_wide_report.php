 ...
 *
 * Usage (CLI):
 *   php SQL/migrations/20260916_copy_mh_report_permissions_to_kdt_wide_report.php
 *
 * Usage (browser, local only):
 *   http://localhost/webJMR/SQL/migrations/20260916_copy_mh_report_permissions_to_kdt_wide_report.php
 *
 * Safe to run multiple times (idempotent). Existing KDT Wide grants are kept.
 * Run 20260916_add_kdt_wide_report_permissions.php first so 56 and 57 exist.
 */
...
const KDT_WIDE_ACCESS = 56;
const KDT_WIDE_ALL_GROUPS = 57;
...
try {
    ...
    $required = [
        ...
        KDT_WIDE_ACCESS => 'KDT Wide Training Report Access',
        KDT_WIDE_ALL_GROUPS => 'KDT Wide Training Report All Group Access',
    ];
    ...
    $beforeAccess = countGrants($connkdt, KDT_WIDE_ACCESS);
    $beforeAll = countGrants($connkdt, KDT_WIDE_ALL_GROUPS);

    $insertedAccess = copyPermission($connkdt, MH_ACCESS, KDT_WIDE_ACCESS);
    $insertedAll = copyPermission($connkdt, MH_ALL_GROUPS, KDT_WIDE_ALL_GROUPS);

    migrateOut('SUCCESS: MH Report permissions copied to KDT Wide Training Report.', $isCli);
    migrateOut(
        sprintf(
            '  Access 3 -> 56: inserted %d (now %d users; was %d)',
            $insertedAccess,
            countGrants($connkdt, KDT_WIDE_ACCESS),
            $beforeAccess
        ),
        $isCli
    );
    migrateOut(
        sprintf(
            '  All groups 51 -> 57: inserted %d (now %d users; was %d)',
            $insertedAll,
            countGrants($connkdt, KDT_WIDE_ALL_GROUPS),
            $beforeAll
        ),
        $isCli
    );
