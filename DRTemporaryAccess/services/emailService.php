...
function sendUnlockRequestSubmittedEmail(array $request, array $toEmailMap, array $ccEmailMap, array $nameMap): bool
{
    ...
    $cc = array_values(array_diff($cc, $toEmails));
    ...
    error_log('sendUnlockRequestSubmittedEmail: final to=' . json_encode($toEmails));
    error_log('sendUnlockRequestSubmittedEmail: final cc=' . json_encode($cc));
...
<body style="margin:0; padding:0; background:#f3f5fb; font-family:Segoe UI,Arial, Helvetica, sans-serif; color:#25324b;">
    <table bgcolor="#eeedf8" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#eeedf8; margin:0; padding:32px 0;">
    	<tr>
    	  <td align="center">
          <table bgcolor="#f7f6fd" role="presentation" width="840" cellpadding="0"cellspacing="0" border="0" style="width:840px;max-width:840px; background: #f7f6fd; border:1px solid #d9ddf1; border-radius:22px; overflow:hidden;">
            ...
            <tr>
              <td bgcolor="#f7f6fd" style="padding:30px 36px 30px 36px; background: #f7f6fd">
                ...
                <table bgcolor="#FFFFFF" role="presentation" width="100%"cellpadding="0" cellspacing="0" border="0"style="border:1px solid #d9ddf1; border-radius:16px;overflow:hidden; background:#FFFFFF;">
                	<tr>
                    <td bgcolor="#F1F6FF" style="padding:18px 20px; background:#F1F6FF; border-bottom:1px solid #E2E8F5;">
                ...
                <table bgcolor="#EFF3FF" role="presentation" width="100%"cellpadding="0" cellspacing="0" border="0"style="margin-top:14px; background:#EFF3FF;border:1px solid #D8E1F5; border-radius:12px;">
                  <tr>
            ...
            <tr>
              <td bgcolor="#f7f6fd" style="padding:0 36px 30px 36px; background: #f7f6fd; border-bottom-left-radius: 22px; border-bottom-right-radius: 22px;">
                <table bgcolor="#f5f8ff" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f5f8ff; border-top:1px solid #e1e9f5; border-radius:0 0 14px 14px;">
                  <tr>
...
function sendSystemEmail(array $payload): bool
{
    ...
    try {
        ...
        $toList = array_values(array_unique(array_filter(array_map(
            fn($email) => trim((string)$email),
            is_array($toList) ? $toList : []
        ))));

        $ccList = array_values(array_unique(array_filter(array_map(
            fn($email) => trim((string)$email),
            is_array($ccList) ? $ccList : []
        ))));

        // Remove anything from CC that already exists in TO
        $ccList = array_values(array_diff($ccList, $toList));

        error_log('sendSystemEmail: payload from=' . $from . ', to=' . json_encode($toList) . ', subject=' . $subject);
        error_log('sendSystemEmail: payload cc=' . json_encode($ccList));

        if ($from === '' || empty($toList) || $subject === '' || $html === '') {
        ...
        error_log('sendSystemEmail: attempting SMTP send to=' . json_encode($toList));

        $mail->send();

        error_log('sendSystemEmail: mail sent successfully to=' . json_encode($toList));
        return true;
    } catch (Exception $e) {
